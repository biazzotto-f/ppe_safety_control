...
<form action="<?= $_ENV['APP_URL'] ?>/usuarios/update" method="POST" enctype="multipart/form-data">
                ...
                <div class="card-body">
                    <div class="text-center">
                        <div class="image-preview-box mb-3 mx-auto">
                            <img src="<?= $usuario['foto_perfil'] ? $_ENV['APP_URL'] . '/' . $usuario['foto_perfil'] : 'https://placehold.co/180x180/e9ecef/6c757d?text=Foto' ?>" class="image-preview" alt="Preview" id="photoPreview">
                        </div>
                    ...
                    <div class="row">
                        <?php foreach ($empresas as $empresa): ?>
                            <div class="col-md-6 mb-3">
                                <label class="company-card-selector <?= in_array($empresa['id'], $empresas_associadas_ids) ? 'selected' : '' ?>">
                                    <img src="<?= $empresa['foto_empresa'] ? $_ENV['APP_URL'] . '/' . $empresa['foto_empresa'] : 'https://placehold.co/80x80/e9ecef/6c757d?text=Logo' ?>" alt="Logo" class="card-logo">
                                    <div class="card-company-name"><?= htmlspecialchars($empresa['nome_empresa']) ?></div>
                                    <input class="form-check-input" type="checkbox" name="empresas[]" value="<?= $empresa['id'] ?>"
                                        <?= in_array($empresa['id'], $empresas_associadas_ids) ? 'checked' : '' ?>>
                                </label>
                            </div>
                        <?php endforeach; ?>
                    </div>
